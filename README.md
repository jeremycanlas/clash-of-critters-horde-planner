...
- Tatari you couldn't drag. If your bench or your field was full, a roster card just wouldn't move: no lift, no message, nothing, which looks like dragging is broken rather than like a rule. Tapping the same card has always said "P1's bench is full (15 max)". Now dragging says it too. Every card picks up; the field and the bench still refuse it, and the refusal is what does the explaining.
- The critter art was pressed flat against the bottom of its square in the roster, so anything drawn near the bottom of its own picture looked like it was falling out. Two changes had met: the sprite lost the inset that used to hold it clear of the edge, and then the effect badges took a strip off the top of its row. It has room on all four sides again.
- Dragging a Tatari off the field back onto the roster stopped working in 1.8.0. The chips got their own drop area inside the roster, and it was sitting in front of the one that takes a critter back. My fault, shipped and caught the same day.
